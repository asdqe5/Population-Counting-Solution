...
import numpy as np #넘파이는 벡터 및 행렬연산에 있어서 엄청난 편의성을 제공한다. matplotlib등의 기반이 되는 라이브러리이다.
import tensorflow as tf #텐서플로는 다양한 작업에 대해 데이터 흐름 프로그래밍을 위한 오픈소스 소프트웨어 라이브러리이다.
from PIL import Image #파이썬 이미징 라이브러리로서 여러 이미지 파일 포맷을 지원하고, 이미지 내부 데이타를 엑세스할수 있게 하며, 다양한 이미지 처리 기능을 제공하고 있다.
import os #os 모듈은 Operating System의 약자로서 운영체제에서 제공되는 여러 기능을 파이썬에서 수행할 수 있게 해줍니다.
from matplotlib import pyplot as plt #그래프를 그리는 기능을 제공한다. 데이터를 모두 효과적으로 도식화한다.
import time #파이썬에서 시간을 다루기 위한 모듈
from glob import glob  #파일들의 목록을 뽑을 때 사용한다. 파일의 경로명을 이용하여 자유롭게 사용.
cwd = os.path.dirname(os.path.realpath(__file__)) #현재작업디렉토리(current working directory), 파일이 위치한 디렉토리
...
class PersonDetector(object):
    def __init__(self):
        ...
        # dataset
        detect_model_name = 'mask_rcnn_inception_v2_coco_2018_01_28'
