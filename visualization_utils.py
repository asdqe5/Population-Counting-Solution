...
"""
import collections #파이썬의 범용 컨테이너 데이터 타입 내장 컨테이너(dict, list, set, tuple)를 대신할 특수 컨테이너 데이터 타입을 제공합니다. 
import functools #higher-order 함수를 위한 것, 
# Set headless-friendly backend.
import matplotlib; matplotlib.use('Agg')  # pylint: disable=multiple-statements / matplotlib는 그래프 그릴 때, 자주 사용하는 package이다. 그래프로 결과를 확인해볼 때, 서버에서는 matplotlib의 명령어가 바로 작동하지 않을 때가 종종있다. 이 때는 window를 띄우지 않고, 바로 영상을 만들어서 저장해야한다.
import matplotlib.pyplot as plt  # pylint: disable=g-import-not-at-top
import numpy as np
import PIL.Image as Image
import PIL.ImageColor as ImageColor #색상 지정자에서 RGB투플까지의 색상 표와 변환기 포함 모듈
import PIL.ImageDraw as ImageDraw #Image 객체에 대한 간단한 2D그래픽을 제공합니다
import PIL.ImageFont as ImageFont #같은 이름의 클래스를 정의, 비트 맵 글꼴을 저장
import six #파이썬 2와 파이썬 3 사이의 차이점을 감쌀 수 있는 단단한 유틸리티를 제공하고 모두에서 수정없이 작동하는 코드 베이스를 지원하기 위한 것이다.
...
def save_image_array_as_png(image, output_path):
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB') #fromarray: numpy 배열을 image 객체로 바꾼다. np.unit8: 부호없는 (양수) 정수형, 8bit, RGV로 전환
